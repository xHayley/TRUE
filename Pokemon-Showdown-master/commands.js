...
var commands = exports.commands = {
	...
	warn: function(target, room, user) {
		...
		this.addModCommand(''+targetUser.name+' was busted sabotaging the rail tracks by '+user.name+'.' + (target ? " (" + target + ")" : ""));
	...
	mute: function(target, room, user) {
		...
		targetUser.popup(user.name+' has caused you to lose the ability to type for 7 minutes. '+target);
		this.addModCommand(''+targetUser.name+' lost the ability to type, thanks to '+user.name+' for 7 minutes.' + (target ? " (" + target + ")" : ""));
		var alts = targetUser.getAlts();
		if (alts.length) this.addModCommand(""+targetUser.name+"'s alts are also unable to type: "+alts.join(", "));
	...
	unmute: function(target, room, user) {
		...
		this.addModCommand(''+targetUser.name+' is able to type again because of '+user.name+'.');
	...
	ban: function(target, room, user) {
		...
		targetUser.popup(user.name+" has hit you with a train.  If you feel that your banning was unjustified you can appeal the ban:\nhttp://www.smogon.com/forums/announcement.php?f=126&a=204\n\n"+target);

		this.addModCommand(""+targetUser.name+" was hit by a Subway Train driven by "+user.name+"." + (target ? " (" + target + ")" : ""));
		var alts = targetUser.getAlts();
		if (alts.length) this.addModCommand(""+targetUser.name+"'s alts were also crushed: "+alts.join(", "));
	...
	unban: function(target, room, user) {
		...
		if (!user.can('ban')) {
			return this.sendReply('/unban - Access to the Subway is denied.');
		}

		var name = Users.unban(target);

		if (name) {
			this.addModCommand(''+name+' came out of the hospital unharmed thanks to '+user.name+'.');
		} else {
			this.sendReply('User '+target+' is not injured.');
		}
	},
	...
	forcerename: function(target, room, user) {
		...
		if (targetUser.userid === toUserid(this.targetUser)) {
			var entry = ''+targetUser.name+' was blessed with the ability to choose a new name by '+user.name+'.' + (target ? " (" + target + ")" : "");
			this.logModCommand(entry);
			...
			targetUser.send('|nametaken||'+user.name+" has been blessed with a new name. You better thank them! "+target);
		} else {
	...
	forcerenameto: function(target, room, user) {
		...
		if (targetUser.userid === toUserid(this.targetUser)) {
			var entry = ''+targetUser.name+' was blessed with a new name '+target+', thanks to '+user.name+'.';
			this.logModCommand(entry);
			Rooms.lobby.sendAuth(entry);
			if (room.id !== 'lobby') {
				room.add(entry);
	...
	lockdown: function(target, room, user) {
		...
		for (var id in Rooms.rooms) {
			if (id !== 'global') Rooms.rooms[id].addRaw('<div class="broadcast-red"><b>The server is going to self-destruct!</b><br />Please finish your battles quickly or you will blow up. No new battles can be started since, its about to explode!</div>');
	...
	endlockdown: function(target, room, user) {
		...
		lockdown = false;
		for (var id in Rooms.rooms) {
			if (id !== 'global') Rooms.rooms[id].addRaw('<div class="broadcast-blue"><b>The self-destruct sequence was aborted! You can carry on living!</b></div>');
		}
